...
D7 完成（170 個測試）。排版引擎、直橫排、拖曳編排、行內編輯器、
四種主力元件與浮動膠囊、Pop-up、md 匯入、縮圖列與目次、教材設定都可用。

接下來：D8 展示用元件（聲音／媒體庫／模組）、AI 佔位、種子檔、
`HANDOVER.md`、乾淨環境驗收。

**兩件事是刻意這樣做的，改的時候要知道**：

- **目次與縮圖都是算出來的，不另外存。** 目次由 `outline.ts` 從角色
  推導，把一段從「中標」改成「內文」目次就少一條；縮圖用的是
  **同一個 `PageView` 縮小**，不是另做一套示意圖。兩套渲染遲早會不
  一樣，而縮圖跟內文不一樣就等於沒有用。量測快取是共用的，所以多畫
  一份縮圖不會讓分頁重算。
- **匯入走 `importDoc` 這個 action，不是換掉整份文件。** 這樣匯入會
  進復原堆疊，老師匯錯檔案能一鍵退回，不必重編一課。
