...
- **編輯用的提示一律絕對定位，不佔版面。** 這一頁裝得下多少是分頁
  ...
  **浮層則更進一步：畫在頁面之外。** 插入面板與浮動膠囊都用 portal 掛到
  body、以視窗座標定位。理由有兩個：頁面是 `overflow: hidden` 的固定畫布，
  留在裡面會被邊緣裁掉（直排被右緣裁、橫排點最下面那一列被下緣裁）；
  而且頁面是整頁等比縮放的，留在裡面字會跟著縮小。
  夾邊界時**量真實尺寸不要用估的**——面板高度會隨內容變。
